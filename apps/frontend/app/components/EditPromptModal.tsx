...
import { PromptDto, PromptFormSchema } from '@app/shared'
import { useModalExpandPreference } from '~/hooks/useModalExpandPreference'

interface EditPromptModalProps {
    prompt: PromptDto
    onSubmit: (id: number, data: { title: string; content: string }) => Promise<void>
}

export function EditPromptModal({ prompt, onSubmit }: EditPromptModalProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            ...
            >
                ...
                >
                    <DialogHeader>
                        <DialogTitle>{t('prompt.editTitle')}</DialogTitle>
                        <DialogDescription>
                            {t('prompt.editDescription')}
                        </DialogDescription>
                    </DialogHeader>
                        ...
                        <form.Field name="title">
                            {(field) => (
                                <div className="flex flex-col gap-2">
                                    <Label htmlFor="edit-title">{t('prompt.nameLabel')}</Label>
                                    <Input
                                        id="edit-title"
                                        name={field.name}
                            ...
                            {(field) => (
                                <div className="flex flex-col gap-2 flex-1 overflow-hidden">
                                    <div className="flex items-center justify-between">
                                        <Label htmlFor="edit-content">{t('prompt.contentLabel')}</Label>
                                        <Button
                                            type="button"
                                            variant="ghost"
                                            size="icon"
                                    ...
                                    <div className="flex-1 overflow-hidden relative">
                                        <Textarea
                                            ...
                                            onBlur={field.handleBlur}
                                            onChange={(e) => field.handleChange(e.target.value)}
                                            placeholder={t('prompt.contentPlaceholderSimple')}
                                        ...
                                        <div className="absolute bottom-2 right-2 text-xs text-muted-foreground bg-background/80 px-2 py-1 rounded">
                                            {field.state.value?.length ?? 0} {t('common.characters')}
                                        </div>
                                    </div>
                                    {field.state.meta.isTouched && field.state.meta.errors.length > 0 && (
                                        <p className="text-sm text-destructive">
