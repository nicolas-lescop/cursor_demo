...
function Home() {
  ...
  const { data: prompts = [], isLoading: isLoadingPrompts } = usePrompts(
    showFavoritesOnly ? true : undefined
  );
  const { data: searchResults, isLoading: isSearching } =
    useSearchPrompts(activeSearch);
              ...
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    >
                      <Star
                        className={`h-4 w-4 transition-colors ${prompt.isFavorite ? 'fill-yellow-400 text-yellow-400' : 'text-muted-foreground hover:text-yellow-400'}`}
                      />
                    </Button>
                    <EditPromptModal
                      prompt={prompt}
